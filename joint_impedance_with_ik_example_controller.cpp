...
Eigen::Vector3d JointImpedanceWithIKExampleController::compute_new_position() {
  ...
  Eigen::Vector3d new_position = position_; // does this need to be initialized? // new_position = position_;
  // fprintf(stderr, "current = %lf,%lf,%lf\n", new_position.x(), new_position.y(), new_position.z());
  std::lock_guard<std::mutex> lock(position_array_mutex);
  auto position_inc = [&](double goal, double& x) {
    double diff = goal - x;
    double sign = 1;
    if (diff < 0) {
      sign = -1;
  ...
  position_inc(seven_dim_pos_goal[0], new_position.x());
  position_inc(seven_dim_pos_goal[1], new_position.y());
  position_inc(seven_dim_pos_goal[2], new_position.z());
  Eigen::Vector3d goal {seven_dim_pos_goal[0], seven_dim_pos_goal[1], seven_dim_pos_goal[2]};

  if (position_.dot(goal) / (position_.norm() * goal.norm()) > 10.999) {
    new_position = position_;
    return position_;
  }

  // new_position.x() += (seven_dim_pos_goal[0] - position_.x())/10000;
  // new_position.y() += (seven_dim_pos_goal[1] - position_.y())/10000;
  // new_position.z() += (seven_dim_pos_goal[2] - position_.z())/10000;
  // fprintf(stderr, "new = %lf,%lf,%lf\n", new_position.x(), new_position.y(), new_position.z());
  // fprintf(stderr, "goal = %lf,%lf,%lf\n", seven_dim_pos_goal[0], seven_dim_pos_goal[1], seven_dim_pos_goal[2]);
  ...
  // fprintf(stderr, "function-of-time delta: %lf, %lf\n", delta_x, delta_z);

  // Eigen::Vector3d new_position = position_;
  // new_position.x() -= delta_x;
  // new_position.z() -= delta_z;

  //position_ = new_position; // SHOULD THIS BE HERE

  return new_position;
}

Eigen::Quaterniond JointImpedanceWithIKExampleController::compute_new_orientation(){
  
  Eigen::Quaterniond new_orientation = orientation_; 
  std::lock_guard<std::mutex> lock(position_array_mutex);

  auto orientation_inc = [&](double goal, double& x) {
    double diff = goal - x;
    double sign = 1;
    if (diff < 0) {
      sign = -1;
  ...
  orientation_inc(seven_dim_pos_goal[3], orientation_.x());
  orientation_inc(seven_dim_pos_goal[4], orientation_.y());
  orientation_inc(seven_dim_pos_goal[5], orientation_.z());
  orientation_inc(seven_dim_pos_goal[6], orientation_.w());
...
controller_interface::return_type JointImpedanceWithIKExampleController::update(
  ...
  Eigen::Quaterniond new_orientation = compute_new_orientation();

  auto service_request =
      create_ik_service_request(new_position, new_orientation /*orientation_*/, joint_positions_current_,
  ...
  auto response_received_callback =
      [this, new_position](ServiceResponseFuture future) {  // NOLINT(performance-unnecessary-value-param)
        const auto& response = future.get();

        if (response->error_code.val == response->error_code.SUCCESS) {
          joint_positions_desired_ = response->solution.joint_state.position;
          // RCLCPP_INFO(get_node()->get_logger(), "Inverse kinaaaaaematics solution succeede!!");
          std::lock_guard<std::mutex> lock(position_array_mutex);
          position_ = new_position;
          // fprintf(stderr, "position_: %f, %f, %f\n", position_.x(), position_.y(), position_.z());
        } else {
          RCLCPP_INFO(get_node()->get_logger(), "Inverse kiaaaaaaanematics solution failed.");
          // std::lock_guard<std::mutex> lock(position_array_mutex);
          // seven_dim_pos_goal[0] = position_.x();
          // seven_dim_pos_goal[1] = position_.y();
          // seven_dim_pos_goal[2] = position_.z();
  ...
  for (int i = 0; i < num_joints_; i++) {
    command_interfaces_[i].set_value(tau_d_calculated(i) / 300);
  }
...
CallbackReturn JointImpedanceWithIKExampleController::on_init() {
  ...
  try {
    fprintf(stderr, "b\n");
    auto ptr = std::make_shared<MinimalSubscriber>(/*this*/);
    std::thread{ [/*this*/ ptr]() {
      fprintf(stderr, "c\n");
      try {
        fprintf(stderr, "d\n");
        ...
        fprintf(stderr, "bad allocation in thread!!\n");
        throw err;
      }
      fprintf(stderr, "f\n");
    }}.detach();
    ...
    fprintf(stderr, "bad in init!!\n");
  }
  // fprintf(stderr, "going away from init\n");
